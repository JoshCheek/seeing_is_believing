      ...
      Open3.popen3 ENV, *popen_args do |process_stdin, process_stdout, process_stderr, thread|
        ...
        event_consumer = Thread.new {
          EventStream::Consumer.new(process_stdout).each do |event|
            ...
            when EventStream::Events::Stdout           then result.stdout             = event.stdout # TODO: rename to value like the others
            when EventStream::Events::Stderr           then result.stderr             = event.stderr # TODO: rename to value like the others
